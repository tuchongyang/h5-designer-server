...
bp.prefix('/form', 'SceneController')
export default class SceneController extends Controller {
    /** 分页列表 */
    @bp.get('/list')
    public async index() {
        const { ctx } = this;
        let list = await ctx.service.form.form.list(ctx.query)
        ctx.success(list)
    }
    @bp.post('/save', auth())
    public async save() {
        ...
        let ret = await ctx.service.form.form.save(params);
        if (ret.code == 0) {
            ctx.success()
        } else {
    ...
    public async update() {
        ...
        let ret = await ctx.service.form.form.update(params);
        if (ret.code == 0) {
            ctx.success()
        } else {
            ctx.fail(ret.message, ret.code)
    ...
    public async publish() {
        ...
        let params = ctx.request.body;
        params.id = ctx.params.id
        let ret = await ctx.service.form.form.publish(params);
        if (ret.code == 0) {
            ctx.success()
        } else {
            ctx.fail(ret.message, ret.code)
    ...
    public async detail() {
        const { ctx } = this;
        const data = await ctx.service.form.form.detail(ctx.params.id)
        if (data) {
    ...
    public async visit() {
        ...
        if (data) {
            ...
            await ctx.service.form.form.update(params);
