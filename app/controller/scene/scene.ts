...
export default class SceneController extends Controller {
  ...
  @bp.get("/homelist")
  public async homelist() {
    const { ctx } = this;
    let list = await ctx.service.scene.scene.homelist(ctx.query);
    ctx.success(list);
  }
  ...
  public async detail() {
    ...
    if (data) {
      const fav = await ctx.model.SceneFav.findOne({
        where: { sceneId: ctx.params.id },
      });
      if (ctx.user) {
  ...
  public async visit() {
    ...
    if (data) {
      ...
      await ctx.service.scene.scene.update(params);
      await ctx.service.scene.statVisit.add({
        sceneId: parseInt(ctx.params.id),
      });
    }
  ...
  public async statTotal() {
    ...
    if (data) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (
          createTime >= new Date(startTimeParam.t).getTime() &&
          createTime <= endTime
        ) {
          result.todayNum += a.pv;
        }
        if (
          createTime >= new Date(startTimeParam.y).getTime() &&
          createTime <=
            new Date(startTimeParam.y.replace("00:00:00", "23:59:59")).getTime()
        ) {
          result.yestodayNum += a.pv;
        }
        if (
          createTime >= new Date(startTimeParam.w).getTime() &&
          createTime <= endTime
        ) {
          result.weekNum += a.pv;
        }
        if (
          createTime >= new Date(startTimeParam.m).getTime() &&
          createTime <= endTime
        ) {
