...
export default class SceneService extends Service {
  ...
  public async save(options: any) {
    ...
    await ctx.model.Scene.upsert(opt)
      .then(() => {
  ...
  public async update(options: any) {
    ...
    data &&
      (await ctx.model.Scene.update(
        { title, desc, cover, music, properties, status, viewCount },
        {
          where: { id: id },
        }
      )
  ...
  public async publish(options: any) {
    ...
    for (let i = 0; i < pages.length; i++) {
      await ctx.model.ScenePage.update(
        { elementsPublish: pages[i].elements },
        { where: { id: pages[i].id } }
      );
    }
    return results;
  ...
  public async remove(id) {
    let results;
    await this.ctx.model.Scene.destroy({ where: { id } })
      .then(() => {
    ...
    //删除所有页面
    await this.ctx.model.ScenePage.destroy({ where: { sceneId: id } });
    return results;
  }
}
