...
export default class FormService extends Service {
    ...
    */
    ...
    public async save(options: any) {
        ...
        await ctx.model.Form.upsert(opt).then(() => {
   ...
   */
    public async update(options: any) {
        ...
        data && await ctx.model.Form.update({ title, desc, cover, music, properties, status, viewCount }, {
            where: { id: id }
        }).then(() => {
            results = { code: 0, message: "添加成功", }
        }).catch(err => {
    ...
    public async publish(options: any) {
        ...
        await ctx.model.Form.update({ status: 2 }, {
            where: { id: id }
        })
        //批量更新
    ...
    public async remove(id) {
        let results
        await this.ctx.model.Form.destroy({ where: { id } }).then(() => {
